...
//	    return -1;
    ...
    [DllImport("libnodave.dll")]
    protected static extern int davePutProgramBlock(IntPtr dc, int blockType, int number, [In, MarshalAs(UnmanagedType.LPArray, ArraySubType = UnmanagedType.U1, SizeParamIndex = 4)] byte[] buffer, int length);
    public int putProgramBlock(int blockType, int number, byte[] buffer, int length)
    {
        return davePutProgramBlock(pointer, blockType, number, buffer, length);
    }

    [DllImport("libnodave.dll")]
    protected static extern int daveInsertBlock(IntPtr dc, int blockType, int number);
    public int InsertBlock(int blockType, int number) {
        return daveInsertBlock(pointer, blockType, number);
    }
